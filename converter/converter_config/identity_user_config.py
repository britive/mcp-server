...
identity_user_management = ToolGroup(
    ...
    tools=[
        ...
        ToolConfig(
            ...
            ai_description="This tool searches for user identities based on a query string. It allows you to find identities by name, username, firstName, lastName, email, type, status, identityProvider or other attributes. The search results include basic details such as userId, name, type, and status. Use this tool to quickly locate user identities that match specific criteria without needing to list all identities.",         
            regenerate=True
            ),
        ToolConfig(
            function_name="enable",
            ai_description="Checks the status of the specified user identity. If the status is inactive, prompts the user for confirmation to enable it. If confirmed, it performs the enable action. If the identity is already active, it informs the user and suggests disabling it instead.",         
            regenerate=True
            ),
        ToolConfig(
            function_name="disable",
            ai_description="Checks the status of the specified user identity. If the status is active, prompts the user for confirmation to disable it. If confirmed, it performs the disable action. If the identity is already inactive, it informs the user and suggests enabling it instead.",
         regenerate=True
        )
    ]
)