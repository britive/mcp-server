...
from converter.converter_config import SYSTEM_PROMPT, TOOLS
from converter.core.logger import LOG_FILE, setup_logging
from converter.core.templates import (
    INIT_FILE,
    RUNNER_FILE,
    get_mcp_init_content,
    get_mcp_runner_content,
)
from converter.core.utils import (
    ...
    get_tool_func_fullname,
    remove_functions_from_content,
    should_generate,
    validate_output_dir,
)
...
def generate_tools_package(generate_all: bool = False, output_dir: str = None) -> None:
    ...
    valid_controllers = [
        controller
        for controller in controller_attrs
        if get_controller_instance(britive, controller)
    ]
    with open(init_file, "w") as f:
        f.write(
            get_mcp_init_content(
                valid_controllers, SYSTEM_PROMPT, output_dir.replace("/", ".")
            )
        )
    ...
    with open(runner_file, "w") as f:
        f.write(
            get_mcp_runner_content(
                init_file.replace(".py", ""), tools_dir, controller_attrs
            )
        )
    ...
    for controller_attr, tools in TOOLS.items():
        ...
        if not controller_instance:
            logger.warning(
                f"⚠️   Controller '{controller_attr}' not found in Britive SDK."
            )
            continue

        controller_file_path = os.path.join(
            tools_dir, f"{controller_attr.replace('.', '_')}.py"
        )

        existing_tool_names = (
            existing_tools(controller_file_path) if not generate_all else set()
        )
        ...
        for tool in tools:
            ...
            if not generate_all and not should_generate(
                tool, tool_func_name, existing_tool_names
            ):
                continue
            ...
            if not method or sdk_func_name.startswith("_"):
                if not method:
                    logger.warning(
                        f"⚠️   Method '{sdk_func_name}' not found in controller '{controller_attr}'."
                    )
                continue

            filtered_params, call_args = extract_params(method)
            new_tool_funcs.append(
                generate_tool_function(
                    tool_func_name,
                    method,
                    controller_attr,
        ...
        if generate_all:
            ...
            tool_list = "\n   • ".join(new_tool_names)
            logger.info(
                f"[✅] Generated '{controller_file_path}' with {len(new_tool_funcs)} tool(s): \n   • {tool_list}"
            )
        else:
            content_to_write = (
                open(controller_file_path).read()
            ...
            if new_tool_funcs:
                ...
                tool_list = "\n   • ".join(new_tool_names)
                logger.info(
                    f"[✅] Updated '{controller_file_path}' with {len(new_tool_funcs)} tool(s): \n   • {tool_list}"
                )
    ...
    if new_tools_count:
        ...
        logger.info(
            "[ℹ️] No new tools generated. Ensure to add new tools in the config."
        )
